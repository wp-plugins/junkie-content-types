 ...
 */
function junkie_types_register_teams_post_type() {
	...
	$args = array(
		'labels' => apply_filters( 'junkie_types_teams_labels', $labels ),
		'supports' => array(
			'title',
			'editor',
			'thumbnail',
	...
	register_post_type( 'teams', apply_filters( 'junkie_types_teams_args', $args ) );

}
add_action( 'init', 'junkie_types_register_teams_post_type' );

/**
 * Change ‘Enter Title Here’ text for the Teams post type.
 */
function junkie_types_change_teams_default_title( $title ) {
	$screen = get_current_screen();

	if ( 'teams' == $screen->post_type ) {
		$title = esc_html__( "Enter the teams's name here", 'junkie-types' );
	}

	return $title;
}
add_filter( 'enter_title_here', 'junkie_types_change_teams_default_title' );

/**
 * Update messages for the Teams admin.
 */
function junkie_types_teams_updated_messages( $messages ) {
	global $post;
	...
	$messages['teams'] = array(
		...
		6  => sprintf( __( 'Team published. <a href="%s">View teams</a>', 'junkie-types' ), esc_url( get_permalink( $post->ID ) ) ),
		7  => esc_html__( 'Team saved.', 'junkie-types' ),
		8  => sprintf( __( 'Team submitted. <a target="_blank" href="%s">Preview teams</a>', 'junkie-types'), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
		9  => sprintf( __( 'Team scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview teams</a>', 'junkie-types' ),
		// translators: Publish box date format, see http://php.net/date
		date_i18n( __( 'M j, Y @ G:i', 'junkie-types' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post->ID ) ) ),
		10 => sprintf( __( 'Team draft updated. <a target="_blank" href="%s">Preview teams</a>', 'junkie-types' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
	);

	return $messages;
}
add_filter( 'post_updated_messages', 'junkie_types_teams_updated_messages' );

/**
 * Change ‘Title’ column label
 * Add Featured Image column
 */
function junkie_types_edit_teams_admin_columns( $columns ) {
	unset( $columns['title'] );
...
add_filter( 'manage_edit-teams_columns', 'junkie_types_edit_teams_admin_columns' );

/**
 * Add featured image to column
 */
function junkie_types_teams_image_column( $column, $post_id ) {
	global $post;
...
add_filter( 'manage_teams_posts_custom_column', 'junkie_types_teams_image_column', 10, 2 );